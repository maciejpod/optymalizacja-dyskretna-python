...
def main(argv):
    ...
    start_node = 0
    end_node = 0
    hint = 'dijkstra.py -n <nodes_number> -s ' \
           '<start_node> -e <end_node>'
    try:
        opts, args = getopt.getopt(argv, "h:n:e:s:",
                                   ["nodes_number=",
                                    "start_node=", "end_node="])
        if len(opts) == 0:
            print(hint)
...
def input_validation(nodes_cnt, start_node, end_node):
    if start_node > nodes_cnt - 1 or end_node > nodes_cnt - 1:
        print('Wezel poczatkowy lub koncowy nie moze '
              'byc wiekszy lub rowny liczbie wezlow')
        sys.exit(2)
    elif nodes_cnt < 1:
